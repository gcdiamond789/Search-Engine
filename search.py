...
def keyword_to_titles(metadata):
    ...
    for row in metadata:
        for keyword in row[4]:
            if keyword in new_dict:
               new_dict[keyword].append(row[0]) 
            else:
                new_dict[keyword] = [row[0]]
    return new_dict
...
def title_to_info(metadata):
    new_dict = {}
    for row in metadata:
        new_dict[row[0]] = {"author": row[1],"timestamp": row[2], "length": row[3]}
    return new_dict
...
def filter_out(keyword, article_titles, keyword_to_titles):
    ...
    new_list = keyword_to_titles[keyword]
    final_list = []
    for article in article_titles:
        if article not in new_list:
...
def articles_from_year(year, article_titles, title_to_info):
    new_list = []
    first_timestamp = datetime.date(year, 1, 1)
    first_unix_timestamp = time.mktime(first_timestamp.timetuple())
    last_timestamp = datetime.date(year, 12, 30)
    last_unix_timestamp = time.mktime(last_timestamp.timetuple())
    
    for article in article_titles:
        if first_unix_timestamp <= title_to_info[article]["timestamp"] <= last_unix_timestamp:
            new_list.append(article)
    return new_list
